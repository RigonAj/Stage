...
| [Trace vs Circle Fitting Comparison Benchmark](perception/method-comparison-benchmark.md) | Headless `method_comparison_benchmark` replaying the Isaac Sim + v2e sequences through both 3D estimators against ground truth: shared-code design (TraceRuntime/SequenceDataset extraction), sequence-local intrinsics and ball radius, one-estimate-per-instant rule, and the RMSE/bias/coverage metrics script. | 2026-08-04 |
