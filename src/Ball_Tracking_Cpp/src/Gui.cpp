...
void Gui::LoadGroundTruthForReader(const std::string &eventPath) {
    ...
    const std::vector<SequenceGroundTruthSample> samples =
        LoadSequenceGroundTruth(groundTruthPath);

    groundTruthTimesSeconds.reserve(samples.size());
    groundTruthWorld3D.reserve(samples.size());
    for (const SequenceGroundTruthSample &sample : samples) {
        groundTruthTimesSeconds.push_back(static_cast<float>(sample.timeSeconds));
        groundTruthWorld3D.push_back({
            static_cast<float>(sample.xCamMeters),
            static_cast<float>(sample.zCamMeters),
            static_cast<float>(-sample.yCamMeters)
        });
    }
